...
while True:
    ...
    def draw_circle(event,x,y,flags,param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN: # checking for if right mouse button clicked 
            drawing = True
            ix,iy = x,y
            ...
            if drawing == True:
                origin=int(((ix+x)/2,(iy+y)/2))
                diameter=((ix+x)**2 + (iy-y)**2)**0.5 #distance between two points
                radius=int(diameter/2)
                
                cv2.circle(img,(ix,iy),(x,y),clr,-1)
